...
class N404ErrorHandlerSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = N404ErrorHandlerHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = N404ErrorHandlerHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!N404ErrorHandlerFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, N404ErrorHandlerFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return N404ErrorHandlerUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = N404ErrorHandlerHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = N404ErrorHandlerHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = N404ErrorHandlerHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new N404ErrorHandlerError($op . "_allow",
                "N404ErrorHandlerSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = N404ErrorHandlerHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new N404ErrorHandlerError("graphql_error",
                "N404ErrorHandlerSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function ErrorHandling($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_error_handling === null) {
                $this->_error_handling = new ErrorHandlingEntity($this, null);
            }
            return $this->_error_handling;
        }
        return new ErrorHandlingEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new N404ErrorHandlerSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
