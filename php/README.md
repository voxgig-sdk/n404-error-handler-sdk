# N404ErrorHandler PHP SDK



The PHP SDK for the N404ErrorHandler API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->ErrorHandling()` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/n404-error-handler-sdk/releases](https://github.com/voxgig-sdk/n404-error-handler-sdk/releases)
...
try {
    ...
    foreach ($errorhandlings as $record) {
...
print_r(array_map(fn($item) => $item->data_get(), $errorhandling));
```
...
```php
$client = N404ErrorHandlerSDK::test($testopts, $sdkopts);
```
...
```
php/
├── n404errorhandler_sdk.php          -- Main SDK class
...
The main class (`n404errorhandler_sdk.php`) exports the SDK class
...
// $errorhandling->data_get() now returns the errorhandling data from the last list
// $errorhandling->match_get() returns the last match criteria
```
