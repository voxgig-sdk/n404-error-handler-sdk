...
// N404ErrorHandler SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class N404ErrorHandlerMakePoint
{
    public static function call(N404ErrorHandlerContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof N404ErrorHandlerError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = N404ErrorHandlerHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = N404ErrorHandlerHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
