...
// N404ErrorHandler SDK retry feature
...
class N404ErrorHandlerRetryFeature extends N404ErrorHandlerBaseFeature
{
    ...
    public function init(N404ErrorHandlerContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (N404ErrorHandlerContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(N404ErrorHandlerContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(N404ErrorHandlerContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
