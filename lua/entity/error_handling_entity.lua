...
local ErrorHandlingEntity = {}
ErrorHandlingEntity.__index = ErrorHandlingEntity


function ErrorHandlingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ErrorHandlingEntity)
  self._name = "error_handling"
  self._client = client
...
function ErrorHandlingEntity:get_name()
  return self._name
end


function ErrorHandlingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ErrorHandlingEntity.new(self._client, opts)
end
...
function ErrorHandlingEntity:mark_deleted()
  self._deleted = true
end


function ErrorHandlingEntity:deleted()
  return true == self._deleted
end


function ErrorHandlingEntity:data_set(args)
  if args ~= nil then
...
function ErrorHandlingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ErrorHandlingEntity:match_set(args)
  if args ~= nil then
...
function ErrorHandlingEntity:match_get()
...
function ErrorHandlingEntity:stream(action, args, callopts)
...
function ErrorHandlingEntity:_run_op(ctx, post_done)
  local utility = self._utility
