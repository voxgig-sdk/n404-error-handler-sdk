...
require("n404-error-handler_types")
...
local N404ErrorHandlerSDK = {}
N404ErrorHandlerSDK.__index = N404ErrorHandlerSDK
...
N404ErrorHandlerSDK._make_feature = _make_feature


function N404ErrorHandlerSDK.new(options)
  local self = setmetatable({}, N404ErrorHandlerSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function N404ErrorHandlerSDK:options_map()
...
function N404ErrorHandlerSDK:get_utility()
  return Utility.copy(self._utility)
end


function N404ErrorHandlerSDK:get_root_ctx()
  return self._rootctx
end


function N404ErrorHandlerSDK:prepare(fetchargs)
...
function N404ErrorHandlerSDK:direct(fetchargs)
...
function N404ErrorHandlerSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function N404ErrorHandlerSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "N404ErrorHandlerSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function N404ErrorHandlerSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function N404ErrorHandlerSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "N404ErrorHandlerSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function N404ErrorHandlerSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = N404ErrorHandlerSDK.new(sdkopts)
  sdk.mode = "test"
