...
local function make_error_util(ctx, err)
  ...
  local msg = "N404ErrorHandlerSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = N404ErrorHandlerError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == N404ErrorHandlerError then
