...
describe("ErrorHandlingEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:ErrorHandling(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:ErrorHandling(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set N404_ERROR_HANDLER_TEST_ERROR_HANDLING_ENTID JSON to run live")
      return
    end
...
function error_handling_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("N404_ERROR_HANDLER_TEST_ERROR_HANDLING_ENTID")
  ...
  if env["N404_ERROR_HANDLER_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["N404_ERROR_HANDLER_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
