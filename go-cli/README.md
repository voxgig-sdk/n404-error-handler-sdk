# n404-error-handler-cli

boru-driven command-line client **and** interactive REPL for the N404ErrorHandler
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/n404-error-handler-cli)
make build

# 2. See usage (words, entities, env vars)
./n404-error-handler-cli --help

# 3. Provide credentials once, via the environment
export N404_ERROR_HANDLER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./n404-error-handler-cli list error_handling

# 5. Override the API base URL for a single call
N404_ERROR_HANDLER_BASE=https://api.example.com ./n404-error-handler-cli list error_handling

# 6. No arguments -> interactive REPL
./n404-error-handler-cli
n404-error-handler> list error_handling
n404-error-handler> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/n404-error-handler-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/n404-error-handler-cli list error_handling
   ```
...
Run with no arguments to open a REPL (prompt `n404-error-handler>`). Each line is
evaluated as its own boru expression:

```text
$ ./n404-error-handler-cli
n404-error-handler> list error_handling
n404-error-handler> /help
n404-error-handler> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `error_handling`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/n404-error-handler-cli`. |
...
(`./n404-error-handler-cli <expr>`) and interactively (the REPL), and expressions compose the
