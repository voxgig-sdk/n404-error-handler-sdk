// Command N404ErrorHandler-cli is a CLI + REPL for the N404ErrorHandler SDK,
...
const prompt = "n404-error-handler"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "error_handling"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      N404_ERROR_HANDLER_APIKEY (api key), N404_ERROR_HANDLER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: N404_ERROR_HANDLER_APIKEY carries the API key and
	// N404_ERROR_HANDLER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("N404_ERROR_HANDLER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("N404_ERROR_HANDLER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewN404ErrorHandlerSDK(opts)
