...
The Python SDK for the N404ErrorHandler API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.ErrorHandling(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.N404ErrorHandler(None).load(
    {"id": "test01"}, None
)
