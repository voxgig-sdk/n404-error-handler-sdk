# N404ErrorHandler SDK test runner
...
class N404ErrorHandlerTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            N404ErrorHandlerTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = N404ErrorHandlerTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = N404ErrorHandlerTestRunner.getenv("N404_ERROR_HANDLER_TEST_LIVE")
        override = N404ErrorHandlerTestRunner.getenv("N404_ERROR_HANDLER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = N404ErrorHandlerTestRunner.getenv(key)
        ...
        explain = N404ErrorHandlerTestRunner.getenv("N404_ERROR_HANDLER_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["N404_ERROR_HANDLER_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if N404ErrorHandlerTestRunner._test_control is not None:
            return N404ErrorHandlerTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                N404ErrorHandlerTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            N404ErrorHandlerTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = N404ErrorHandlerTestRunner.load_test_control()
...
def load_env_local():
    N404ErrorHandlerTestRunner.load_env_local()


def env_override(m):
    return N404ErrorHandlerTestRunner.env_override(m)


def entity_data(v):
    return N404ErrorHandlerTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return N404ErrorHandlerTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return N404ErrorHandlerTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return N404ErrorHandlerTestRunner.load_test_control()


def live_client_options():
    return N404ErrorHandlerTestRunner.live_client_options()


def live_delay_ms():
    return N404ErrorHandlerTestRunner.live_delay_ms()
