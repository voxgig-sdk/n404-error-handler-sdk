...
class TestErrorHandlingDirect:

    def test_should_direct_list_error_handling(self):
        setup = _error_handling_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-error_handling", "live" if setup["live"] else "unit")
        if _skip:
...
def _error_handling_direct_setup(mockres):
