...
class ErrorHandlingEntity
  ...
  def make
    opts = @_entopts.dup
    ErrorHandlingEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = N404ErrorHandlerHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ErrorHandling, Hash] the current ErrorHandling data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = N404ErrorHandlerHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ErrorHandling fields)
  def match_get
  ...
  #
  # @param reqmatch [ErrorHandlingListMatch, Hash, nil] match filter (any subset of
  #   ErrorHandling fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<ErrorHandling>, Array] the matching ErrorHandling items; raises N404ErrorHandlerError on failure
