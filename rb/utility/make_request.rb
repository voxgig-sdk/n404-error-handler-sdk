...
module N404ErrorHandlerUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = N404ErrorHandlerResponse.new({})
    ...
    if fetch_err
      ...
      response = N404ErrorHandlerResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = N404ErrorHandlerResponse.new(fetched)
    else
