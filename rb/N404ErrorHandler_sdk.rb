...
require_relative 'N404ErrorHandler_types'
...
class N404ErrorHandlerSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = N404ErrorHandlerHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = N404ErrorHandlerHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, N404ErrorHandlerFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    N404ErrorHandlerUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = N404ErrorHandlerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = N404ErrorHandlerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = N404ErrorHandlerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue N404ErrorHandlerError => err
    ...
    if fetched.is_a?(Hash)
      status = N404ErrorHandlerHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = N404ErrorHandlerError.new(
        "graphql_error", "N404ErrorHandlerSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = N404ErrorHandlerSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
