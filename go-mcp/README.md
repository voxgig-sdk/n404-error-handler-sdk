...
[MCP](https://modelcontextprotocol.io) server exposing the N404ErrorHandler SDK as
two agent tools — `n404-error-handler_list` and `n404-error-handler_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/n404-error-handler-mcp)
make build

# 2. Provide credentials via the environment
export N404_ERROR_HANDLER_APIKEY=sk_live_xxx
...
```jsonc
// n404-error-handler_list: first page of records
{ "entity": "error_handling" }
{ "entity": "error_handling", "query": { } }

// n404-error-handler_load: one record by id
{ "entity": "error_handling", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/n404-error-handler-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user n404-error-handler \
     -- "$PWD"/dist/*/n404-error-handler-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `n404-error-handler_list` and `n404-error-handler_load` tools now appear
   in new sessions. Ask the agent to *"list error_handling using n404-error-handler"*
   and it calls `n404-error-handler_list` with `{"entity":"error_handling"}`.
...
```sh
export N404_ERROR_HANDLER_APIKEY=sk_live_xxx            # API key
export N404_ERROR_HANDLER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `n404-error-handler_list` tool
...
### Call the `n404-error-handler_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `n404-error-handler_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `n404-error-handler_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `N404_ERROR_HANDLER_APIKEY` | API key sent with every request. |
| `N404_ERROR_HANDLER_BASE` | Optional override of the API base URL. |
