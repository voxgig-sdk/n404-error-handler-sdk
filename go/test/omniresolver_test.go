...
import (
	...
	omni "github.com/voxgig-sdk/n404-error-handler-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.N404ErrorHandlerError); is {
				out := map[string]any{
					"name":    "N404ErrorHandlerError",
...
func omniCtx(arg any, client *sdk.N404ErrorHandlerSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.N404ErrorHandlerSDK); is {
			client = live
