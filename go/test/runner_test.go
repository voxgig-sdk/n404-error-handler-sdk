...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("N404_ERROR_HANDLER_TEST_LIVE") == "TRUE" ||
		os.Getenv("N404_ERROR_HANDLER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("N404_ERROR_HANDLER_TEST_EXPLAIN"); explain != "" {
