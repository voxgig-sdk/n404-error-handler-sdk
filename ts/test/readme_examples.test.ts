...
//     (\`new N404ErrorHandlerSDK(...)\` / \`N404ErrorHandlerSDK.test(...)\` become
//     \`N404ErrorHandlerSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'N404ErrorHandlerSDK'
...
const TEST_SEED = {"entity":{"error_handling":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new N404ErrorHandlerSDK(...)\` and any
// \`N404ErrorHandlerSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+N404ErrorHandlerSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/N404ErrorHandlerSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
