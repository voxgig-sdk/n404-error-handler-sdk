...
describe('ErrorHandlingDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new N404ErrorHandlerSDK({
  ...
  test('direct-list-error_handling', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-error_handling', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['N404_ERROR_HANDLER_TEST_ERROR_HANDLING_ENTID']
