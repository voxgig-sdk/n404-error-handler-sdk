...
// N404ErrorHandler SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class N404ErrorHandlerTestFeature extends N404ErrorHandlerBaseFeature
{
    ...
    public function init(N404ErrorHandlerContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (N404ErrorHandlerContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = N404ErrorHandlerParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (N404ErrorHandlerContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(N404ErrorHandlerContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = N404ErrorHandlerParam::call($ctx, $k);
