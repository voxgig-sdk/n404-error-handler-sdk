...
//    only - a typed N404ErrorHandlerContext there would make every ctx
...
final class N404ErrorHandlerOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new N404ErrorHandlerContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new N404ErrorHandlerSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new N404ErrorHandlerResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new N404ErrorHandlerError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new N404ErrorHandlerResponse($clean['response']);
    ...
    public static function err_from_map($m): ?N404ErrorHandlerError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new N404ErrorHandlerError(is_string($code) ? $code : '', $msg);
    }
