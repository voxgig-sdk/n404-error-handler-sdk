...
// N404ErrorHandler SDK feature test
//
...
class FtCtrl extends N404ErrorHandlerControl
{
...
class FtClient
{
    ...
    private N404ErrorHandlerUtility $utility;

    public function __construct(N404ErrorHandlerUtility $utility, array $options)
    {
    ...
    public function get_utility(): N404ErrorHandlerUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (N404ErrorHandlerContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public N404ErrorHandlerContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (N404ErrorHandlerContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = N404ErrorHandlerConfig::make_feature($name);
    ...
    private static function buildUrl(N404ErrorHandlerSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof N404ErrorHandlerError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new N404ErrorHandlerResponse($fetched);
        }
    ...
    private function fail(N404ErrorHandlerContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (N404ErrorHandlerContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
