...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = N404ErrorHandlerOmni.make_runner(TEST_JSON_FILE, N404ErrorHandlerSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    N404ErrorHandlerError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : N404ErrorHandlerOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = N404ErrorHandlerOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
