...
module N404ErrorHandlerOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if N404ErrorHandlerOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = N404ErrorHandlerOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| N404ErrorHandlerOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || N404ErrorHandlerOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = N404ErrorHandlerSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = N404ErrorHandlerError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { N404ErrorHandlerOmni.errify(err) }
